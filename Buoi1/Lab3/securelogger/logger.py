...
def verify_log_integrity():
    """So sánh từng dòng log với chữ ký SHA-256; trả về danh sách số dòng bị sửa."""
    with open(LOG_FILE, encoding="utf-8") as f:
        lines = f.read().splitlines()
    with open(SIGNATURE_FILE, encoding="utf-8") as f:
        sigs = f.read().splitlines()
    # Sau khi rotate, file .sig chứa cả chữ ký của log cũ -> lấy phần cuối
    sigs = sigs[-len(lines):] if lines else []
    tampered = [i + 1 for i, (line, sig) in enumerate(zip(lines, sigs)) if hash_line(line) != sig]
    if len(sigs) != len(lines):
        tampered.append("line count mismatch")
    return tampered
...
class JSONFormatter(logging.Formatter):
    def format(self, record):
        ...
        record_dict = {
            # Dùng record.created để format nhiều lần vẫn ra cùng 1 dòng (khớp chữ ký)
